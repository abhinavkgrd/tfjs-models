...
export type ObjectDetectionTrainingDataset = 'coco' | 'open_images';
...
export async function load(config: ModelConfig = {}) {
    ...
    const dataset = config.dataset || 'coco';
    const modelUrl = config.modelUrl;
    if (
        ['mobilenet_v1', 'mobilenet_v2', 'lite_mobilenet_v2'].indexOf(base) ===
        -1
    ) {
        throw new Error(
    ...
    if (['coco', 'open_images'].indexOf(dataset) === -1) {
        throw new Error(
    ...
    if (dataset === 'open_images' && ['mobilenet_v2'].indexOf(base) === -1) {
        throw new Error(
    ...
    if (dataset === 'open_images' && modelUrl === null) {
        throw new Error(
...
export class ObjectDetection {
    ...
    private dataset: ObjectDetectionTrainingDataset;
    ...
    private buildDetectedObjects(
        ...
        for (let i = 0; i < count; i++) {
            ...
            objects.push({
                ...
                class: CLASSES[this.dataset][classes[indexes[i]] + 1]
                    .displayName,
